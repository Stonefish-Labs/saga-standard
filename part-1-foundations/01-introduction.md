...
## 1.1 The Secret Isolation Problem in Agentic AI
...
This is the **three-party problem**: the human authorizes, the agent decides, and the tool consumes -- but the agent, which sits between authorization and consumption, cannot be trusted with the secrets it is orchestrating the use of.
...
*Why this matters:* The human owns the secrets, the consequences of their use, and the liability for the actions of agents acting on their behalf. No amount of delegation removes the human from the accountability chain. No level of convenience justifies removing the owner's ability to control their own credentials.
...
*Why this matters:* An agent that can invoke a tool has access to the full scope of that tool's capabilities -- restricting what the tool can *do* is outside the scope of this standard. What this standard *can* control is which secrets the tool receives. Scoped access is the principle of least privilege applied to the one dimension the secret management layer controls: which credentials reach which tool. Compartmentalization limits blast radius. If one tool is compromised, damage is contained to its authorized profiles.
...
*Why this matters:* In the event of a breach or misuse, you need to know exactly what was accessed and by what.

### Goal 6: The Standard Must Be Implementable in Any Runtime

The standard must not require specific programming languages, operating systems, or frameworks. A conformant implementation must be achievable with common operating system primitives (process isolation, filesystem permissions, IPC), standard cryptographic libraries, and standard inter-process communication mechanisms. The standard does not require any specific operating system, though specific transport bindings (see [Conformance](../part-4-conformance/13-conformance.md)) may leverage platform-specific facilities such as Unix domain sockets or Windows named pipes.

*Why this matters:* Agentic systems are heterogeneous. The standard must work across Python tools, Node.js servers, Go services, and anything else the ecosystem produces -- on any platform that supports process isolation and standard cryptography.
